...
function maskToBinary(mask: boolean[]): string {
...
export function emit(gv: GValue, options: EmitOptions = {}): string {
  return emitValue(gv, options);
}

function emitValue(gv: GValue, opts: EmitOptions): string {
  switch (gv.type) {
    case 'null':
      return canonNull();
    case 'bool':
...
function emitList(gv: GValue, opts: EmitOptions): string {
  const items = gv.asList().map(v => emitValue(v, opts));
  return '[' + items.join(' ') + ']';
}

function emitMap(gv: GValue, opts: EmitOptions): string {
  const parts: string[] = [];
...
function emitStruct(gv: GValue, opts: EmitOptions): string {
...
function emitSum(gv: GValue, opts: EmitOptions): string {
  const sum = gv.asSum();
  if (sum.value === null) {
    return `${sum.tag}()`;
  }
...
export function emitPacked(gv: GValue, schema: Schema, options: PackedOptions = {}): string {
  if (gv.type !== 'struct') {
...
function shouldUseBitmap(gv: GValue, td: TypeDef, schema: Schema): boolean {
...
function emitPackedDense(gv: GValue, td: TypeDef, schema: Schema, opts: PackedOptions): string {
...
function emitPackedBitmap(gv: GValue, td: TypeDef, schema: Schema, opts: PackedOptions): string {
...
function getFieldValue(gv: GValue, fd: FieldDef): GValue | null {
  const sv = gv.asStruct();
...
function isFieldPresent(val: GValue | null, fd: FieldDef): boolean {
  if (val === null) return false;
  if (val.type === 'null' && fd.optional && !fd.keepNull) return false;
  return true;
}

function emitPackedValue(gv: GValue, schema: Schema, opts: PackedOptions): string {
  switch (gv.type) {
    case 'null':
      return canonNull();
    case 'bool':
...
export function emitTabular(gv: GValue, schema: Schema, options: TabularOptions = {}): string {
  if (gv.type !== 'list') {
...
export function emitHeader(options: HeaderOptions = {}): string {
...
export function emitV2(gv: GValue, schema: Schema, options: V2Options = {}): string {
...
function selectMode(gv: GValue, schema: Schema, tabThreshold: number): 'struct' | 'packed' | 'tabular' {
