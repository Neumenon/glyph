...
- JavaScript includes GS1 stream framing (`stream` sub-export) and the `StreamingValidator`. Python also has full GS1 stream framing (`glyph.stream`) and the streaming validator, but no `pack` surface.
