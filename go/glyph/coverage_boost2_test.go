...
// ============================================================
// writeValueWithRefs coverage — bytes, time, ID, blob, poolref, sum, struct
// ============================================================

func TestWriteValueWithRefs_Bytes(t *testing.T) {
	gv := Bytes([]byte{0xDE, 0xAD})
	opts := DefaultAutoPoolOpts()
	result, err := AutoPoolEncode(List(gv), opts)
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "b64") {
		t.Errorf("expected b64 in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_Time(t *testing.T) {
	tm := time.Date(2026, 3, 9, 12, 0, 0, 0, time.UTC)
	gv := Time(tm)
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "2026") {
		t.Errorf("expected time in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_ID(t *testing.T) {
	gv := IDFromRef(RefID{Prefix: "U", Value: "123"})
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "U") {
		t.Errorf("expected ID ref in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_Blob(t *testing.T) {
	gv := BlobFromContent([]byte("hello"), "text/plain", "", "")
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if result.Output == "" {
		t.Error("expected non-empty output for blob")
	}
}

func TestWriteValueWithRefs_Sum(t *testing.T) {
	gv := Sum("Ok", Int(42))
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	...
	if !strings.Contains(result.Output, "Ok") {
		t.Errorf("expected sum tag in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_Struct(t *testing.T) {
	gv := Struct("Point", MapEntry{Key: "x", Value: Int(1)}, MapEntry{Key: "y", Value: Int(2)})
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "Point") {
		t.Errorf("expected struct name in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_Nil(t *testing.T) {
	// List with a nil item
	gv := List(nil, Int(1))
...
func TestWriteValueWithRefs_PoolRef(t *testing.T) {
	gv := &GValue{typ: TypePoolRef, poolRef: &PoolRef{PoolID: "S1", Index: 0}}
	result, err := AutoPoolEncode(List(gv), DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
...
func TestWriteValueWithRefs_Float(t *testing.T) {
	// Test float zero and negative zero
	gv := List(Float(0), Float(-0.0), Float(3.14))
	result, err := AutoPoolEncode(gv, DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "3.14") {
		t.Errorf("expected 3.14 in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_Bool(t *testing.T) {
	gv := List(Bool(true), Bool(false))
	result, err := AutoPoolEncode(gv, DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	}
	if !strings.Contains(result.Output, "t") || !strings.Contains(result.Output, "f") {
		t.Errorf("expected t and f in output, got %q", result.Output)
	}
}

func TestWriteValueWithRefs_NullType(t *testing.T) {
	gv := List(Null())
	result, err := AutoPoolEncode(gv, DefaultAutoPoolOpts())
	if err != nil {
		t.Fatalf("AutoPoolEncode: %v", err)
	...
	if result.Output == "" {
		t.Error("expected non-empty output for null list")
	}
}

func TestWriteValueWithRefs_StringRef(t *testing.T) {
	// Test string that becomes a pool ref
	longStr := "this string is repeated many times for pooling"
	items := make([]*GValue, 5)
	for i := range items {
		items[i] = Map(MapEntry{Key: "data", Value: Str(longStr)})
	}
	gv := List(items...)
	result, err := AutoPoolEncode(gv, DefaultAutoPoolOpts())
	if err != nil {
	...
	// Should have a pool definition
	if !strings.Contains(result.Output, "@pool") {
		t.Errorf("expected pool definition, got %q", result.Output)
	}
}
...
// ============================================================
// pool.go — parsePoolEntries, DefaultAutoInternOpts at 0%
// ============================================================

func TestDefaultAutoInternOpts(t *testing.T) {
	opts := DefaultAutoInternOpts()
	if opts.MinLength <= 0 {
		t.Errorf("expected positive MinLength, got %d", opts.MinLength)
	}
}
